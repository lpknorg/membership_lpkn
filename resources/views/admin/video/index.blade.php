	...
	$(document).ready(function(){
		...
		$('body').on('click', '[id^="btnShow"]', function(e){
			...
			$.ajax({
				...
				error: function(data) {
					showAlert("Ada kesalahan dalam melihat data video", "error")
				}
			});
		})
		...
		function showModal2(act, data=null){
			...
			if(act == 'show' || act == 'edit'){
				...
				$('#modalShow [name=judul]').val(data.video.judul)
				$('#modalShow [name=link]').val(data.video.link)
				$('#modalShow [name=keterangan]').val(data.video.keterangan)
			}
			if (act == 'add') {
				$('#modalAdd').modal('show')
			}else if (act == 'show') {
				$('#modalShow .modal-title').text('Lihat Data video')
				$('#modalShow #btnUpdate').hide()
				$('#modalShow input, #modalShow select').attr('disabled', true)
			}else{
				$('#modalShow .modal-title').text('Ubah Data video')
				$('#modalShow #btnUpdate').show()
				$('#modalShow input, #modalShow select').attr('disabled', false)
				$('#modalShow form').attr({
					action: `${window.location}/${data.video.id}`,
					method: 'POST'
				})
			}
		}
