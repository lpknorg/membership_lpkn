	...
	$(document).ready(function(){
		...
		function showModal2(act, data=null){
			...
			if (act == 'add') {
				...
				$('#modalShow .modal-title').text('Lihat Data artikel kategori')
				$('#modalShow #btnUpdate').hide()
				$('#modalShow input, #modalShow textarea').attr('disabled', true)
			}else{
				$('#modalShow .modal-title').text('Ubah Data artikel kategori')
		...
		var table = $('#table-Datatable').DataTable({
			processing: true,
			serverSide: true,
			ajax: "{{ route('admin.artikel_kategori.dataTables') }}",
			columns: [
