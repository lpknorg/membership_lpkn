	...
	$(document).ready(function(){
		...
		$('body').on('click', '[id^="btnLihat"]', function(e) {
			...
			$.ajax({
				...
				error: function(data) {
					showAlert("Ada kesalahan dalam melihat data Instansi", "error")
				}
			});
		})
		...
		function showModal2(act, data = null) {
			...
			if (act == 'add') {
				...
				$('#modalShow .modal-title').text('Lihat Data Instansi')
				$('#modalShow #btnUpdate').hide()
				$('#modalShow input, #modalShow textarea').attr('disabled', true)
			} else {
				$('#modalShow .modal-title').text('Ubah Data Instansi')
		...
		var table = $('#table-Datatable').DataTable({
			processing: true,
			serverSide: true,
			ajax: "{{ route('admin.instansi.dataTables') }}",
			columns: [{
				data: 'DT_RowIndex',
				name: 'DT_RowIndex',
				orderable: false,
